...
<!--      <option value="1" v-for="a in Att">{{a}}</option>-->
    ...
    <input type="date" class="date-box" id="did"  v-model="date" >
...
export default {
  ...
  data() {
    return {
      ...
      yData: ['Saturday', 'Friday', 'Thursday', 'Wednesday', 'Tuesday', 'Monday', 'Sunday'],
  ...
  methods: {
    selAtt() {
      let sAtt = document.getElementById('pid').value;
      let Att = ['溶解氧','pH值','水温','营养盐','透明度']
      this.zText = Att[sAtt];
      let sDate = document.getElementById('did').value;
      this.date = sDate.day;
      this.yData[6] = sDate;
      alert(sDate)
    }
